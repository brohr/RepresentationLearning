...
class ImageNetDataProviderRotation(data.TFRecordsParallelByFileProvider):
    """
    ...
    def __init__(self,
        ...
        super(ImageNetDataProviderRotation, self).__init__(
            source_dirs,
            postprocess=postprocess,
            **kwargs
        )

        self.train_rotations = np.random.randint(0, 4, N_TRAIN)
        self.val_rotations = np.random.randint(0, 4, N_VAL)
    ...
    def postproc_imgs(self, ims):
        ...
        if self.group == 'train':
            ...
            rotations = self.train_rotations[self.train_index : self.train_index + self.batch_size]
            ims = tf.map_fn(lambda x: tf.image.rot90(x[0], x[1]), (ims, rotations))
            ...
            rotations = self.val_rotations[self.val_index : self.val_index + self.batch_size]
            ims = tf.map_fn(lambda x: tf.image.rot90(x[0], x[1]), (ims, rotations))
